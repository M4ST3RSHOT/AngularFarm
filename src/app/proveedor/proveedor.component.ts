...
export class ProveedorComponent {
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(CrearProveedorComponent, {
      data: { proveedor: proveedor, texto: 'REGISTRAR PROVEEDOR' },
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      this.proveedorServ.agregar(proveedor).subscribe(
        (data) => {
          this.proveedor = data;
  ...
  actualizar(item: Proveedor) {
    let proveedor: Proveedor;
    const dialogRef = this.dialog.open(CrearProveedorComponent, {
      data: { proveedor: item, texto: 'EDITAR PROVEEDOR' },
    });
    dialogRef.afterClosed().subscribe((result) => {
      proveedor = {
        id: item.id,
      ...
      this.proveedorServ.actualizar(proveedor, item.id).subscribe(
        (data) => {
          this.proveedor = data;
  ...
  visualizar(item: Proveedor) {
    const dialogRef = this.dialog.open(DetallarProductoProveedorComponent, {
      data: item,
    });
  }
}
