...
export class NuevaFacturaComponent implements OnInit {
  ...
  get descuento() {
    return this.descuento_info.get('descuento');
  }
  ...
  ngOnInit(): void {
    ...
    this.cliente.listar().subscribe((data) => {
      ...
      this.opcionesFiltradascliente =
        this.opcionControlcliente.valueChanges.pipe(
          startWith(''),
  ...
  private _filtrarclientes(value: string): Cliente[] {
    ...
    return this.opcionescliente.filter((cliente) =>
      cliente.ci.toString().toLowerCase().includes(filtro)
  ...
  setear_descuento(a1: any) {
    ...
    a = parseInt(a1);
    this.desc = a * 0.01;
    this.formulario_factura.total =
      this.formulario_factura.subtotal -
      this.formulario_factura.subtotal * this.desc;
  ...
  carrito() {
    let formulario_detalle = {
      id: 0,
      producto_id: 0,
      cantidad: 0,
      factura_id: 0,
    };
    let formulario_producto = {
      id: 0,
      codigo: '',
      nombre: '',
      descripcion: '',
    ...
    formulario_producto.codigo = this.form.codigo;
    ...
    this.form.subtotal =
      formulario_detalle.cantidad * formulario_producto.precio_venta;
  ...
  realizar_venta() {
    ...
    for (let i = 0; i < this.ventas.length; i++) {
      ...
      let info = { id: '', stock: '' };
      matrizinfodetalle.cantidad = this.ventas[i][6];
    ...
    this.formulario_factura.cliente_id.toString();

    this.info_factura.cliente_id =
      this.formulario_factura.cliente_id.toString();
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.factura.agregar(this.info_factura).subscribe(
          (data) => {
            for (let i = 0; i < this.matrizdetalle.length; i++) {
              ...
              this.detalle.agregar(a).subscribe(
                (data) => {},
                (error) => {
                  if (error.status == 500)
            ...
            this.toastr.success('Exito', 'Venta realizada');
            setTimeout(() => {
          ...
          (error) => {
            if (error.status == 500)
              this.toastr.error('Revise su conexión', 'Error');
            else this.toastr.error('Error al agregar la factura', 'Error');
          }
        );
        for (let i = 0; i < this.matrizproducto.length; i++) {
          this.producto.mostrar(this.matrizproducto[i].id).subscribe((data) => {
            ...
            this.producto.actualizar(a, a.id).subscribe(
              (data) => {},
              (error) => {
                if (error.status == 500)
                  this.toastr.error('Revise su conexión', 'Error');
  ...
  crearysetearcliente() {
    ...
    const dialogRef = this.dialog.open(CrearClienteComponent, {
      data: { cliente: cliente, texto: 'REGISTRAR NUEVO CLIENTE AGILIZADO' },
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      cliente = {
        ...
        fecha_nacimiento: this.fecha1,
        ci: result.value.ci,
        correo: 'desconocido@desconocido',
        telefono: '0000000',
      ...
      this.nuevocliente.controls['nombre'].setValue(cliente.nombre);
      this.nuevocliente.controls['apellido'].setValue(cliente.apellido);
      this.nuevocliente.controls['ci'].setValue(cliente.ci);
    });
  }
}
